...
Environmental stress was still an important predictor in our model. 
...
We found that some species, there is a strong negative relationship between stress and specialisation, while for others, there is a strong positive relationship (Figure \@ref(fig:fig-random-effects)a). 
Interestingly, the slope of these relationships was correlated with the species' specialisation intercepts in the model (Figure \@ref(fig:fig-random-effects)b and c). The mean correlation coefficient was `r round(random_cor$Estimate, 2)` [`r round(random_cor$Q2.5, 2)`, `r round(random_cor$Q97.5, 2)`] (95% credible intervals shown within square brackets).
...
The the variability of the specialisation-stress relationship across species was also evident when looking at overall patterns. 
...
The standard deviation of the species intercepts was `r round(sd_org_id$Estimate, 2)` [`r round(sd_org_id$Q2.5, 2)`, `r round(sd_org_id$Q97.5, 2)`], and that of the species' stress slope was `r round(sd_org_id_suitability$Estimate, 2)` [`r round(sd_org_id_suitability$Q2.5, 2)`, `r round(sd_org_id_suitability$Q97.5, 2)`] . 
