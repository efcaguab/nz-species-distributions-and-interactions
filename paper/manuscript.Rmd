...
# n_sp_correct_in_data %>%
#   group_by(queried_sp_name) %>%
#   count(sort = TRUE)

# sp_incorrect_in_data <- checked_sp_names %>%
```
...
Out of those, `r format(n_sp_names_sp_level, big.mark = ",")` were originally identified to the species-level, `r format(n_sp_names_gen_level, big.mark = ",")` to the genus level, and no taxonomic information was avaliable for the restant `r format(n_no_tax_info, big.mark = ",")`. 
We were able to confirm the validity of `r format(n_sp_correct_in_data, big.mark = ",")` of the scientific names of organisms identified to the species level (roughly `r scales::percent(n_sp_correct_in_data/n_sp_names_sp_level, 1)`).
