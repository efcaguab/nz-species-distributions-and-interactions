...
Species with many partners, conversely, should have a larger pool of available partners and might, therefore, be more likely to specialise on the most benefitial partners under environmental stress. 
...
Specifically we used the `dudi.pca` function from the R package `ade4` `r as.character(packageVersion('ade4'))` [@dray_ade4_2007] and center and scale all bioclimatic variables to have a mean of zero and a unit variance. 
...
The normalised density at that particular location [which we calculated using the R package `raster` `r as.character(packageVersion('raster'))`; @hijmans_raster_2019] corresponds the bioclimatic suitability. 
