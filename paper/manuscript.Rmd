...
Specifically we use the `dudi.pca` function from the R package `ade4` `r as.character(packageVersion('ade4'))` [@dray_ade4_2007] and center and scale all bioclimatic variables to have a mean of zero and a unit variance. 
...
Interestingly, the slope of this relationship correlates negatively with the species' intercept in the model (Figure \@ref(fig:fig-random-effects)b and c). 
The mean correlation coefficient was `r round(random_cor$Estimate, 2)` [`r round(random_cor$Q2.5, 2)`, `r round(random_cor$Q97.5, 2)`].
In other words, the slope of the stress-specialisation relationship was more likely to be positive for species with a smaller number of partners in low-stress conditions (and negative for species with a larger number of partners).

```{r}
loadd(random_slope_intercepts)
# random_slope_intercepts <- readRDS("rsi.rds")

species_types <- random_slope_intercepts %>%
  # dplyr::rename_all(stringr::str_replace, "Q75", "Q97.5") %>%
  # dplyr::rename_all(stringr::str_replace, "Q25", "Q2.5") %>%
  # = Q75, Q2.5 = Q25) %>%
  dplyr::mutate(intercept_sig = dplyr::case_when(Intercept_Q2.5 > 0 ~ "positive", Intercept_Q97.5 < 0 ~ "negative", TRUE ~ "indeterminate"), 
                slope_sig = dplyr::case_when(scaled_suitability_Q2.5 > 0 ~ "positive", scaled_suitability_Q97.5 < 0 ~ "negative", TRUE ~ "indeterminate"), 
                int_slo = interaction(intercept_sig, slope_sig), 
                int_slo = dplyr::case_when(int_slo == "indeterminate.indeterminate" ~ "neither generalist or specialist", int_slo == "positive.indeterminate" ~ "true generalist", int_slo == "negative.indeterminate" ~ "true specialist", int_slo %in% c("positive.positive", "negative.negative") ~ "flexible species", TRUE ~ "flexible species") ) %>%
  dplyr::mutate(int_slo = forcats::fct_infreq(int_slo)) 

species_types %>%
  dplyr::count(int_slo, guild) %>%
  tidyr::spread(guild, n)

  # dplyr::filter(intercept_sig != "indeterminate") %>%
  # table(slope_sig, intercept_sig)

library(ggplot2)
species_types %>%  # 
  ggplot(aes(x = int_slo, fill = guild)) +
  geom_bar() +
  coord_flip()
```
...
There was also a large difference on WAIC between the model that included this predictor and that that excluded it. 
...
The standard deviation of the species intercept was `r round(sd_org_id$Estimate, 2)` [`r round(sd_org_id$Q2.5, 2)`, `r round(sd_org_id$Q97.5, 2)`], and that of the species' suitability slope was `r round(sd_org_id_suitability$Estimate, 2)` [`r round(sd_org_id_suitability$Q2.5, 2)`, `r round(sd_org_id_suitability$Q97.5, 2)`] (95% credible intervals shown within square brakets). 
...
Although both niche and neutral processes are relevant in determining species interactions, our model suggests that niche processes may be the predominant mechanism through which the environment *systematically* affect species degree. 
