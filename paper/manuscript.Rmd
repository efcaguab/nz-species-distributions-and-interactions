...
It is widely awknowledged that there are two main processes involved in whether two species in a community interact or not.
The first is an "Eltonian" niche process that is underpinned by the traits of the interacting species.
Under this rationale, when the traits of interacting species are optimally matched, due for example to a deep-rooted co-evolutionary history, species are able to obtain a high benefit from the interaction. 
Contrastingly, we can expect the per-capita benefit of interaction with a relatively low trait matching to be lower.
The second is a neutral process underpinned by the abundance of the intereacting species. 
All else being equal, when the abundance of a species is high their potential encounter rate is also high and therefore there is a higher probability to interact than if it were rare. 

Perhaps, because it operates at scales larger than those commonly considered by community ecologists, the abiotic environment is one of the factors that is commonly overlooked when explaining interaction turnover [but see @gravel_bringing_2018]. 
The most evident, and best studied, way through which the environment can afect the structure of ecological networks is by way of patterns of community composition. 
This is because the abiotic environment can strongly influence whether a species can have a positive growth-rate when rare in a particular location.
