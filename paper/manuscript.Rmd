...
To determine the environmental space of each species we use the first two components from a principal component analysis of the 37 bioclimatic variables associated with the species occurrences. 
...
We then estimate a kernel density function to determine a smooth density of occurrences in the environmenta space. 
The main advantage of using a kernel smoother is that it reduces the sensitivity of the niche to sampling effort and the resolution chosen for the environmental space grid [@broennimann_measuring_2012]. 
...
We normalise the occurrence density such that our suitability ranges beetween 1 for the grids with environmental conditions in which the species was most commonly observed and 0 for the grids in which it was never observed.  
