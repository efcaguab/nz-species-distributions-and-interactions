...
Therefore, our first step was to ensure consistent spelling and standarisation of species names synonyms (See Supplementary Methods).
...
Our models performed relativelly well.
...
Overall we found that environmental suitability does not show a consistent pattern accross species. 
Indeed, when looking at the population level effects, suitability has virtually no relationship with normalised degree, neither for plants or pollinators (Figure \@ref(fig:fig-conditional-effects)b). 
...
Interestingly, the slope this relationship was negatively correlated with the species' intercept in the model (Figure \@ref(fig:fig-random-effects)b). 
The mean correlation coefficient was `r round(random_cor$Estimate, 2)` [`r round(random_cor$Q2.5, 2)`, `r round(random_cor$Q97.5, 2)`].
In other words, the slope of the suitability-degree relationship was more likelt to be positive for species with a smaller number of partners in lower-suitability and negative for species with a larger number of partners in lower-suitability conditions.
...
This result indicates that the availability of potential partners accounts for a large proportion of the variabiity on species degree.  
Importantly, our findings about the suitability-degree relationship were qualitatively unchanged whether this variable was included or not. 
...
We set out to explore wether and how environmental stress can systematically affect species degree. 
After accounting for the pool of potential partners, we found that indeed environmental conditions contribute to determining wether a species is a generalist or a specialist in their community. 
However, we also found that the particular effect of environment is strongly dependent on the species.
We proposed two altenative hypotheses of how environmental stress may affect degree and we found evidence for both of them.
...
In summary, environmental stress pushes species that are flexible enough to change their interaction partners towards interpediate levels of degree, a so called "regression towards the mean".

Our results suggest that changes in community composition are indeed the primmary channel through which environment determines changes interaction probability. 
However, they also show that, for a large number of species, the environment may also play a substantial role on determining their realised (Eltonian) niche. 
...
We believe that part of this difficulty could have arisen because species, and ultimately network structre, can respond multiple, and contrasting, ways depending on the particular bioclimatic variable being examined (e.g. temperature or precipitation).
...
Second, even if there is a relationship between suitability and abundances, a particular environmental gradient could have a positive effect in the abundance some species and a negative effect in others. 
Indeed, we show that within a community there is a wide range of suitability values, even for the relatively limited number of species we were able to include in our analyis. 

Recent research suggest that species are continuoisly chaning there interaction partners wherever environmental conditions change in space of time [@raimundo_adaptive_2018].
So far it appears that this rewiring is primarly driven by generalist species [@ponisio_opportunistic_2017; @burkle_plant-pollinator_2013], presumably because generalist species are less sensitive to trait matching of their interaction partners [@caradonna_interaction_2017].
...
While our model detected how the environment affects interaction of two types of species, generalists and flexible specialists, there is a third group that remained invisible for our model but has important implications for network persistence and stability.
It has been shown that species that are able to flexibly vary their interaction partners and their role in the network are more likely to persist in their community as environmental conditions vary [@gaiarsa_interaction_2019].
We propose this third group of is composed by specialists that are constrained to interact with parterns of high trait-matching and therefore were not likely to be found in more than one community. 
...
Similarly, if the patterns we see in our models have also played a role during the evolutionary history of pollination communities, our results also help explain why only a small fraction of plant-pollinator interactions show a strong signature of deep co-evolutionary history.
The increasses on stress that species are predicted to experience due to rapid environmental change might further erode the co-evolutionary history of specialist species.
Communities as a whole might be set in a trayectory of even more diffuse co-evolution.
