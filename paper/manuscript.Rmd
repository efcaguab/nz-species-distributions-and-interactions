...
From a mutualistic perspective, ecological interactions are thought to be selected based on the relative benefit they provide. 
On one end of the spectrum, generalist species have traits that allow them to interact with a large number of species.
In average, the benefits they obtain from each of the species they interact with is relatively low compared to specialists.
On the other end of the spectrum, specialist species are thought to have a more deep-rooted co-evolutionary history with their partner species. 
This tighter co-evolution, in turn, can be reflected on a better trait-matching with the species it interacts with which leads to a higher benefit.
This "Eltionian" niche-based rationale can also operate within a species. 
We can expect that a species can obtain a higher benefit from interactions with species with traits that better match their requirements. 

Although several other factors, chiefly neutral processes determined by abundance, can also influence the probability of whether two species interact or not, one that is commonly overlooked by community ecologists is the role of the abiotic environment [but see @gravel_bringing_2018]. 
The most studied mechanism is through patterns of community composition. 
The abiotic environment can strongly influence whether a species can have a positive growth-rate when rare in a particular location.
For two species to interact, they first need to co-occur in the same community, which can only happen if the environmental conditions of the community fall within the Fundamental "Grinellian" niche of both species. 
However, community composition is only one part of the story as the abiotic environment is known to be able to influence the interactions directly [@tylianakis_ecological_2017]. 
...
It is generally agreed that two species wether two species interact there is a neutral process
 ...
 small number of partner species are thought to benefit largely from each  deeper co-evolutionary history that, through trait matching
Species with a small number of partners, specialists, are often deemed to 
Species with a large number of interactions, generalists, benefits relatively 
