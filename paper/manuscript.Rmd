...
<!-- Variation in community composition can be independent of variation of ecological interactions, suggesting that species and inter- action distribution may well respond to diferent drivers (Poisot et al. 2012). -->
...
We then used a series of bayesian multilevel models to evaluate the impact of environmental suitability on the number of interacting species. 
These models differed on the predictors included and allowed us to evaluate the relative importance of suitability. 
Our response variable was the number of species it interacts with in a community, the species degree. 
We modelled degree using a logarithmic link function and a Poisson distribution. 
Nevertheless results were qualitatively the same if instead we model the normalised degree [@martin_gonzalez_centrality_2010] using a binomial distribution in which degree is the number of successes and the number of species in the opposite guild is the number of trials (Supp info). 

Briefly, our baseline model included suitability and guild as population effects and two groups, species and the community. 
Our main predictor, the environmental suitability of the species in that particular location, is an index defined betwen zero and one in which a value of zero indicates that the species has never been observed in this environment while a value of one indicates that the species is most commonly observed in that environment. 
As we are particularly intrested on understanding wether the effect of species suitability is conditional on the species guild (plant or pollinator), we also included guild and its interaction with suitability in the model. 
...
Finally, to account for unmeasured differences between communities, like sampling effort, sampling method, or richness, we calculated an intercept for each community in our study.

We calculated three more alternative models. 
The first alternative model accounted for the overall generalism of a species by also including among the predictors the magnitude of the total number of interacting species across networks. 
In the second alternative model, we included as a predictor the number of known possible interacting species. 
We calculated this predictor as the number of interacting species across networks that were present in the community. 
Accounting for the number of possible interacting species is a particularly stringent way to test the effect of environmental suitability as for many species populations, the number of possible interacting species is the same or very similar to the number of species it actually interacts with. 
The third alternative model included both the overall species generalism and the possible number of interacting species. 
Finally, the fourth model included both of the aforementioned predictors but excluded the effect of environmental suitability both at the population and the species level. 
...
```
Call:
glm(formula = n_partners ~ scale(suitability) * guild, family = "poisson", 
    data = .)

Coefficients:
                               Estimate Std. Error z value Pr(>|z|)    
(Intercept)                     1.79092    0.02360  75.886  < 2e-16 ***
scale(suitability)              0.07750    0.02454   3.158  0.00159 ** 
guildpla_id                     0.64345    0.04361  14.756  < 2e-16 ***
scale(suitability):guildpla_id  0.07645    0.03631   2.106  0.03524 *  
---
AIC: 3739.6
```
...
```
Call:
glm(formula = n_partners ~ scale(suitability) * guild + scale(log(n_partners_global)), 
    family = "poisson", data = .)

Coefficients:
                               Estimate Std. Error z value Pr(>|z|)    
(Intercept)                     1.70709    0.02541  67.172  < 2e-16 ***
scale(suitability)              0.17021    0.02481   6.860 6.88e-12 ***
guildpla_id                     0.61872    0.04378  14.132  < 2e-16 ***
scale(log(n_partners_global))   0.42074    0.01970  21.362  < 2e-16 ***
scale(suitability):guildpla_id -0.02134    0.03657  -0.584     0.56    
---
...
Call:
glm(formula = n_partners ~ scale(suitability) * guild + scale(log(n_partners_global)) + 
    scale(n_possible_partners), family = "poisson", data = .)

Coefficients:
                               Estimate Std. Error z value Pr(>|z|)    
(Intercept)                    1.667769   0.025701  64.892  < 2e-16 ***
scale(suitability)             0.088729   0.024121   3.679 0.000235 ***
guildpla_id                    0.423815   0.043904   9.653  < 2e-16 ***
scale(log(n_partners_global))  0.138837   0.022829   6.082 1.19e-09 ***
scale(n_possible_partners)     0.351851   0.008547  41.167  < 2e-16 ***
scale(suitability):guildpla_id 0.010224   0.036728   0.278 0.780718    
---
