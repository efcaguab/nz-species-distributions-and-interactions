...
As our aim is to compare the throphic niche for different suitability levels, we only do this for species that were present in at least `r numbers2words(readd(minimum_spp_locations))` communities.
...
This approach is based on the probabiliy density function of a species distribution in an environmental variable space. 
In a nutshell, habitats (characterised by a collection of environmental variables) in which the species ocurrs most often are deemed to be suitable for the species than habitats in which the species has never been observed. 
This approach to estimate the habitat suitability requires to key pieces of information. 
...
We then complemented data obtained from WorldClim with data from Envirem [@title_envirem_2018], which includes 16 extra bioclimatic and two topographic variables. 
The additional set of variables from Envirem have been shown to be relevant to ecological or physiological processes and as such have the potential to improve our suitability estimation. 
...
After obtaining information about the species occurrence and the evironment, the next step was to merge these two datasets such that the location of each occurrence is characterized by a vector with information of our 37 bioclimatic and topographic variables. 
...
As expected, a vast majority of these problematic cells were close to the shore. 
To address this issue, we calculated the average value of environmental variables within a `r config$climate_buffer/1000`km buffer of the center of the cell where the variable is missing, and used it to approximate the value of the variable in that cell. 
...
Next, we calculate the probability density function of the species distribution in environemntal space. 
...
We use a kernel method to estimate this density and normalise it such that it ranges beetween zero and one. 
Specifically, to calculate the probability density function we use `ecospat.grid.clim.dyn` from the R package `ecospat` `r as.character(packageVersion('ecospat'))` [@ecospat_citation] with a grid resolution of 200.
We then determine the location in environmental space of the plant-pollinator communities where the species is present using the function `suprow` from `ade4`.
The normalised density at that particular location corresponds to our suitability metric, which we calculate using the R package `raster` `r as.character(packageVersion('raster'))` [@raster_citation]. 
We use the kernel density method in the niche-factor analyisis [@broennimann_measuring_2012] rather than the distance from the mode [@hirzel_ecological-niche_2002], as it has been proposed earlier, as it has been shown to reduce the procedure's sensitivity to sampling effort and the resolution of the environmental space.
...
Normalised degree was modelled using a logit link function and a binomial distribution in which the number of species interacts with is the number of successes and the number of species in the opposite guild is the number of trials. 
We are aware that wether species intreacts or not is not a bernoulli process as species interactions are not strictly independent from each other, however a binomial distribution allow us to indirectly account for the differences in species richness across communities. 
Importantly, however, results are qualitatively the similar when we model species degree directly using a Poisson distribution and a logarithmic link function (Supp info).

Within each model set we evaluate four models in order to evaluate the relative importance of suitability. 
...
The population-level predictors in the baseline model, commonly called fixed effects, were the habitat suitability, the species guild (whether it was a plant or a pollinator), a metric of the species generalism, and its number of known possible partners.
...
Specifically, we estimated a species overal level of generalism as the total number of interacting species across communities in our dataset. 
We also included the number of known possible partners as a predictor in our models as it allow us to control for the environmental effects on species co-occurrence. 
We calculate the number of possible partners by determining the number of species in a community with which the species is known to interact in any other location in our dataset. 
Controlling for the number of potential partners makes our model a particularly stringent test of our environemntal stress hypotheses as this number is often the same, or very close, to the number of actual partners, particularly for species in a small number of communities or with a small number of interactions.  
As we were particularly intrested on understanding wether the effect of habitat suitability is conditional on the species guild (plant or pollinator), we therefore included guild and its interaction with suitability in the model. 
...
First, it allow us to inspect the extent to which suitability is a population or a group level effect. 
Second, by investigating the correlation between the intercept and the slope as a model parameter it allowed us to inspect the extent by which species with a small or large number of interactions respond to increassing levels of environmental stress. 
...
We compare this baseline model with three alternative models in which we remove one predictor at a time.
