...
There are multiple examples of species that interact with a large number of partners in an ecological community or a particular season but with fewer in another. 
It has been proposed that at least some of this variation can be attributed to environmental drivers. 
...
However, how exactly the environment, and the stress it imposes on species, affects wether two species interact or not, and ultimately species degree the number of partners a species has---its degree---is still unknown. 
It is crucial to understand why and how species degree might change because it underpins species importance and, collectively is the foundation of network structure that underpins ecosystem function and stability. 
...
Both of these processes can be influenced by the environment. 
Therefore, it is not surprising that multiple studies have shown how changes to interactions can be driven by environmental change [@tylianakis_ecological_2017].
For instance, some studies show that the rate of some trophic interactions, like predation [@mckinnon_lower_2010; @vucic-pestic_warming_2011] and herbivory [@baskett_latitudinal_2018], can increase with temperature but might decrease with precipitation [@pires_predicted_2016]. 
Contrastingly, other studies have shown either no overall effect or non-linear effects of temperature or precipitation on plant-pollinator interactions at the community level [@devoto_patterns_2005; @gravel_bringing_2018].

Each species can have multiple partners. 
Each of these partners, as well as the interactions with them, can be simultaneoulsy affected by the environmental conditions. 
Given all these sources of variation, it is not clear whether the environment can systematically affect species degree. 
Previous research suggests that there might be two alternative hypothesis of how the environment, in particular the environmental stress, affects the number of partners of a species [@tylianakis_ecological_2017]. 
