...
As the species level was the most common taxonomic rank available in our interaction datasets, in all further analysis, we grouped together subspecies or varieties within a same species. 
...
These spelling mistakes were corrected automatically by fuzzy matching the canonical names in our data sources with those in the Global Names Resover database. 
However, in rare occasions, the fuzzy matching algorithm can suggest a cannonical name that has similar spelling but that correspond to an organism in a different taxonomic group, often a different kingdom. 
To address this potential problem, we checked the taxonomic hierarchy of suggested names and confirmed that it matched to our expectated taxon.
...
As species names are constantly changin, we subsequently checked for possible synonyms of the canonnical names in our data sources. 
...
Finding these alternative names was important for two main reasons. 
...
This can ocurr not only when the cannonical name has been changed but also when there are widely used ortographic variants.
...
We then manually consulted multiple online databases, chiefly Wikispecies (https://species.wikimedia.org/), and looked for cannonical names that both, resembled the unvalidated names and matched the geographic and taxonomic spectations. 
