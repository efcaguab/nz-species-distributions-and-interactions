...
```{r}
loadd(spp_to_download, gbif_keys, n_occurrences)
...
n_species_with_clean_occurrences <-  dplyr::full_join(spp_to_download, gbif_keys, 
		 by = c("sp_name" = "queried_sp_name")) %>%
...
This resulted in `r round(sum(n_cleaned_occurrences$N)/1000000, digits = 1)` million occurrences for `r n_species_with_cleaned_occurences` species. 
