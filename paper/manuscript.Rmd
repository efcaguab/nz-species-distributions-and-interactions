...
Similarly, @lavandero_genotype_2013 found that environmental stress due to higher temperature reduced the trophic niche breadth of parasitoids suggesting higher specialisation. 
...
First, we calculated the envoronmental stress of species in their communitues.
Second, we relate the species stress in their communitu with the number of partner species it has as a metric of their level of specialisation.
...
We assume that stress a species experience in a particular location is inversely related to the suitability of the average environmental conditions in that place. 
...
We removed from further analysis occurrences located within the `r complete_filled_cells_2[1]` cells for which we were unable to fill environmental variables (`r scales::percent(complete_filled_cells_2[1]/sum(complete_filled_cells_2))` of the total). 
...
Specifically we use the `dudi.pca` function from the R package `ade4` `r as.character(packageVersion('ade4'))` [@dray_ade4_2007] and center and scale all bioclimatic variables to have a mean of zero and a unit standard deviation. 
...
The normalised density at that particular location [which we calculate using the R package `raster` `r as.character(packageVersion('raster'))`; @hijmans_raster_2019] corresponds the bioclimatic suitability. 
