...
However, suitability is still an important predictor as the difference between our baseline model and that that did not include suitability was XXX (±XX) (±SE). 
...
The slope of this relationship was negatively correlated with the species' intercept in the model (Figure \@ref(fig:fig-random-effects)b). 
The correlation coefficient was `r round(random_cor$Estimate, 2)` with a `r round(random_cor$Q2.5, 2)` to `r round(random_cor$Q97.5, 2)` 95% credible interval.
...
Random effects SD. eviation

```{r fig-conditional-effects, fig.width = width('single'), fig.height = 2.2*3, fig.cap = "Conditional effects"}
...
```{r fig-random-effects, fig.width = width('single'), fig.height = 2.2*2.5, fig.cap = "Random effects"}
