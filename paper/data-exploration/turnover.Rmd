...
```{r, eval = F}
selected <- int_rank %>%
  dplyr::select(pla_name, pol_name, n_loc, focal) %>% 
  tidyr::gather("guild", "sp_name", pla_name, pol_name) %>% 
  dplyr::mutate(guild = substr(guild, 1,3),
                focal_int = TRUE) %>% 
  dplyr::group_by(guild, sp_name) %>%
  dplyr::summarise(n_loc_int = mean(n_loc), 
                   n_loc_max = max(n_loc),
                   focal_int = any(focal), 
                   n = n()) %>% 
  dplyr::distinct() %>%
  dplyr::left_join(sp_rank) 

selected %>% 
  # dplyr::filter(focal, focal_int) %>%
  ggplot(aes(x = n_loc, y = n_loc_int)) +
  geom_point(aes(colour = log(n))) +
  geom_label_repel(data = dplyr::filter(selected, focal, focal_int),
